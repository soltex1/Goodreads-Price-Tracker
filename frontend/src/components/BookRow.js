...
const storeNames = new Map([
  ['bertrand', 'Bertrand'],
  ['book_depository', 'Book Depository'],
  ['wook', 'Wook'],
  ['fnac', 'Fnac']
])
    ...
    <td style={{ minWidth: '42px'}}>{Object.values(prices).map(({ value, uri }, index) => {
      return <div key={index} className="storePrice">{
        value === null
          ...
          : typeof value === 'string'
          ? <p>{value}</p>
          : <a target="_blank" href={uri} className={min === value ? 'min' : 'normal'}><p>{value}</p></a>}
      </div>
    })}
    </td>
    <td>{Object.values(prices).map(({ value, uri }, index) => {
      return <div key={index} className="storePrice euroSymbol">{
        value !== null && value !== 'N/A' && <p className={min.value === value ? 'min' : 'normal'}>€</p>}
      </div>
    })}
    </td>
  </tr>
